...
class cilaApp(object):
    ...
    def inviaSmtpMail(self,obj,ObjectId,to=[],cc=[],bcc=[]):
        ...
        iDoc = IolApp(doc)
        diz_mail = iDoc.getConvDataMail('mail_%s' %('cila'))
        ...
        allegati = {}
        if bodyMail.get('attach') != "":
            if bodyMail.get('attach') in doc.getItems():
                
                allegati.update({key:doc.getfile(filename=key) for key in doc.getItem(bodyMail.get('attach')).keys()})
            

        
        res = IolDocument(doc).smtpMail(data = diz_mail['config'],to = args['to'],oggetto = oggetto,testo = testo, allegati=allegati,cc=[],bcc=[])
        return res  
...
class cilaWsClient(object):
    ...
    def getProcedimento(self, obj):
        ...
        if 'atti_acquisiti' in tipo:
            ...
            pr.tipo = 21300
        if 'art21bis_versamento_data' in doc.getItem('qualifica_interventi_opt',[]):
            pr.tipo = 21320
